...
export const ExperienceDetail = ({
  ...
  const reserver = () => {
    ...
    toast.success(now ? `Réservé : ${exp.titre} 🎉` : "Réservation annulée");
  };
      ...
      <SheetContent side="bottom" className="rounded-t-3xl max-h-[92vh] overflow-y-auto p-0">
        ...
        <div className="px-5 -mt-12 relative pb-6">
          <SheetHeader className="text-left">
            ...
            <SheetTitle className="text-2xl leading-tight">{exp.titre}</SheetTitle>
          </SheetHeader>
          ...
          <div className="grid grid-cols-3 gap-2 mt-4">
            ...
            <div className="bg-gradient-hero rounded-xl p-2.5 text-center text-primary-foreground">
              ...
              <p className="text-[10px] opacity-90">/ pers.</p>
            </div>
          </div>
          ...
          {exp.galerie.length > 1 && (
            <section className="mt-5">
              <h4 className="text-xs uppercase tracking-wider font-bold text-primary mb-2">Galerie</h4>
          ...
          <div className="sticky bottom-0 -mx-5 mt-6 px-5 py-3 bg-background/95 backdrop-blur-md border-t border-border">
            ...
            >
              {reserved ? <><Check className="h-5 w-5 mr-2" />Réservé</> : "Réserver"}
            </Button>
          </div>
        </div>
      </SheetContent>
