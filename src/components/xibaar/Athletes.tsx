    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-primary font-semibold">
          {a.age} {t("common.yearsOld", { defaultValue: "ans" })} • {t(`common.countries.${a.pays.toLowerCase()}`, { defaultValue: a.pays })}
        </p>
      </div>
      <button
...
export const Athletes = () => {
  ...
  const filtered = athletes.filter((a) => {
    ...
    if (!tq) return true;
    return `${a.prenom} ${a.nom} ${a.pays} ${a.discipline}`.toLowerCase().includes(tq);
  });
    ...
    <div className="space-y-3">
      <div className="relative">
        ...
        <Input
          ...
          placeholder={t("xibaar.ui.searchAthletes")}
          maxLength={60}
          className="pl-9 h-11 rounded-xl bg-card"
        />
      </div>
      <p className="text-xs text-muted-foreground">
        {t("xibaar.ui.athletesCount", { count: filtered.length })}
      </p>
